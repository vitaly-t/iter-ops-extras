 ...
 * and `Array.flatMap` precisely, in case such behavior is preferred.
 *
 * The library's own implementation expands all iterables, including strings.
 * This implementation simply makes strings non-expandable.
 */

export function flat<T>(depth = 1) {
    return _flat(depth, v => typeof v === 'string'); // use v3.2.0 'skip' callback
}
